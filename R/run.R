#' @importFrom shiny shinyApp uiOutput renderUI observeEvent reactiveValues reactive debounce
#' @importFrom shiny actionButton sliderInput selectInput radioButtons textInput
#' @importFrom shiny conditionalPanel verbatimTextOutput downloadHandler
#' @importFrom shiny tags div p h5 h6 hr br img tagList HTML renderText
#' @importFrom bslib page_sidebar sidebar accordion accordion_panel
#' @importFrom bslib navset_card_tab nav_panel bs_theme
#' @importFrom magick image_read image_write image_scale image_info
#' @importFrom magick image_channel image_convert image_modulate image_negate
#' @importFrom magick image_enhance image_normalize image_rotate image_flip
#' @importFrom magick image_flop image_trim image_blur image_threshold
#' @importFrom magick image_quantize image_level image_fill image_morphology
#' @importFrom magick image_edge image_despeckle
...
apply_operation <- function(image, op_name, params) {
  tryCatch({
    switch(op_name,
           ...
           level = image_level(image, black_point = as.integer(params$black_point), white_point = as.integer(params$white_point)),
...
ui <- page_sidebar(
  ...
  # ---- SIDEBAR ----
  sidebar = sidebar(
    h5("Momoshop", style = "font-weight: 700; letter-spacing: 0.5px; margin-bottom: 20px;"),
  ...
  navset_card_tab(
    ...
    nav_panel("Code",
              div(style = "padding: 20px;",
                  ...
                  actionButton("dl_btn", "Download", class = "btn btn-info btn-sm")
              )
    )
  ),
    ...
    lapply(seq_along(rv$pipeline), function(i) {
      ...
      div(class = if (step$enabled) "pipeline-node" else "pipeline-node muted",
        ...
        div(class = "node-actions",
              actionButton(paste0("pipeline_toggle_", step$id), if (step$enabled) "look" else "dont look", class = "action-btn", title = if (step$enabled) "Disable" else "Enable", onclick = sprintf("Shiny.setInputValue('pipeline_toggle_click', %d);", step$id)),
              actionButton(paste0("pipeline_edit_", step$id), "->", class = "action-btn", title = "Edit", onclick = sprintf("Shiny.setInputValue('pipeline_edit_click', %d);", step$id)),
              if (i > 1) actionButton(paste0("pipeline_up_", step$id), "up", class = "action-btn", title = "Move up", onclick = sprintf("Shiny.setInputValue('pipeline_up_click', %d);", step$id)),
              if (i < length(rv$pipeline)) actionButton(paste0("pipeline_down_", step$id), "down", class = "action-btn", title = "Move down", onclick = sprintf("Shiny.setInputValue('pipeline_down_click', %d);", step$id)),
              actionButton(paste0("pipeline_delete_", step$id), "remove", class = "action-btn", title = "Delete", onclick = sprintf("Shiny.setInputValue('pipeline_delete_click', %d);", step$id))
          )
      )
    })
  })
